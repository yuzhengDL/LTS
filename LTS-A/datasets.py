...
def load_dataset(name='f8k', load_train=True):
	...
	train_ims = train_ims.astype(numpy.float32)
    else:
        train_ims = None
    dev_ims = numpy.load(loc+name+'_dev_ims.npy')
    dev_ims = dev_ims.astype(numpy.float32)
    test_ims = numpy.load(loc+name+'_test_ims.npy')
    test_ims = test_ims.astype(numpy.float32)

    return (train_caps, train_ims), (test_caps, test_ims), (dev_caps, dev_ims)
